...
void CSoundMgr::SetPlayeSpeed()
{
	for (int i = 0; i < MAXCHANNEL; ++i)
	{
		if (i == SOUND_ATTACK || i== SOUND_SLOW)
			continue;

		FMOD_Channel_SetFrequency(m_pChannelArr[i], 44100.f * CTimeMgr::Get_Instance()->Get_TimeScale());
		FMOD_System_Update(m_pSystem);
	}

}

void CSoundMgr::SetPause(CHANNELID eID, bool bPause)
{
